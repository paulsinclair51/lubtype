...
{ const xchar_t hay[] = {'a','b','a','b','a',0};
  ...
  assert(xxsnstrm(hay, 16, needle, '|', -1) == hay + 2);
  assert(xxsnSTRM(hay, 16, needle, '|', -1) == hay + 2);
}
...
{ const xchar_t hay[] = {'a','b','a','c','a','d',0};
  ...
  assert(xxsnstrm(hay, 16, set, 0, -1) == hay + 5);
  assert(xxsnSTRM(hay, 16, set, 0, -1) == hay + 5);
}
...
{ const xchar_t empty[] = {0};
  ...
  assert(xxsnstrm(empty, 4, one_delim, '|', 1) == empty);
  assert(xxsnstrm(one, 4, empty, 0, 1) == NULL);
  assert(xxsnstrm(one, 4, empty, 0, 1) == NULL);
}
 ...
 */
void LUB_PASTE(run_cmp_search_tests_, LUB_X)(void)
{ xchar_t s1[16] = {'a','b','c','\0'};
  xchar_t s2[16] = {'a','b','c','\0'};
  xchar_t s3[16] = {'a','b','d','\0'};

  // Compare.
  assert(xxsnncmp(s1, 3, s2, 3) == 0);
  assert(xxsnncmp(s1, 3, s3, 3) < 0);
  ...
  { const xchar_t hay_count[] = {'a','b','c','a','b','c',0};
    ...
    assert(xxsncnt(hay_count, 6, needle_count, '|') == 2);
  }
